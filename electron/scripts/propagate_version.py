...
def update_version_in_package_json(version):
    ...
    package_content = re.sub(
        r'"version": "\S+"', f'"version": "{version}"', package_content
    )
...
def update_version_in_pyproject_toml(version):
    ...
    pyproject_content = re.sub(
        r'version = "\S+"', f'version = "{version}"', pyproject_content, 1
    )
...
def update_version_in_frontend_package_json(version):
    ...
    frontend_package_content = re.sub(
        r'"version": "\S+"', f'"version": "{version}"', frontend_package_content
    )
