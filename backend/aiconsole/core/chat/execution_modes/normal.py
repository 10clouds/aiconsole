...
async def render_materials_from_message_group(
    message_group: AICMessageGroup, context: ExecutionModeContext, agent: Agent
...
async def execution_mode_normal(
    ...
    context.chat_mutator.op_create_message(
        message_group_id=context.message_group.id,
    ...
    async for chunk in gpt_executor.execute(
        ...
        if chunk == CLEAR_STR:
            ...
            context.chat_mutator.op_append_to_message_content(
                message_id=message_id, content_delta=chunk["choices"][0]["text"]
            )
