...
class AICConnection:
    ...
    async def send(self, msg: BaseServerMessage):
        await self._websocket.send_json(
            {"type": msg.get_type(), **msg.model_dump(mode="json")}
        )
...
async def send_message_to_chat(
    ...
    for connection in _active_connections:
        if (
            chat_id in connection.open_chats_ids
            and connection != source_connection_to_ommit
        ):
            await connection.send(msg)


async def send_message_to_all(
    msg: BaseServerMessage, source_connection_to_ommit: AICConnection | None = None
):
